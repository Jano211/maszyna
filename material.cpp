...
bool opengl_material::deserialize(cParser &Input, bool const Loadnow)
{
	...
	{
		// record current texture path in the material, potentially needed when material is reloaded
		// on environment change NOTE: we're storing this only for textures that can actually
		// change, to keep the size of path database modest
...
void opengl_material::finalize(bool Loadnow)
{
	...
	if (parse_info)
	{
		for (auto it : parse_info->tex_mapping)
		{
			...
			else if (key.size() > 2)
			{
				...
				{
					// ignore unrecognized texture bindings in legacy render mode, it's most likely
					// data for more advanced shaders
	...
	for (auto it : shader->texture_conf)
	{
		...
		// NOTE: texture validation at this stage relies on forced texture load behaviour during its
		// create() call
		// TODO: move texture id validation to later stage if/when deferred texture loading is
		// implemented
...
bool opengl_material::update()
{
	...
	cParser materialparser(
	    name + ".mat",
	    cParser::buffer_FILE); // fairly safe to presume .mat is present for branching materials
...
std::unordered_set<std::string> seasons = {"winter:", "spring:", "summer:", "autumn:"};
...
std::unordered_set<std::string> weather = {"clear:", "cloudy:", "rain:", "snow:"};
...
bool opengl_material::deserialize_mapping(cParser &Input, int const Priority, bool const Loadnow)
{
	...
	if (Priority != -1)
	{
		...
		else if (key.compare(0, 7, "texture") == 0)
		{
			...
			if (it == parse_info->tex_mapping.end())
				parse_info->tex_mapping.emplace(
				    std::make_pair(key, parse_info_s::tex_def({value, Priority})));
		...
		else if (key == "depth_mask:" &&
		         (!depth_mask.has_value() || Priority > m_depth_mask_priority))
		{
			std::string value = Input.getToken<std::string>(true, "\n\r\t;");
			if (!value.empty())
			{
				...
				{
					depth_mask = true;
					m_depth_mask_priority = Priority;
				}
				...
				{
					log_error("illegal depth mask value: " + value);
				}
			}
		}
		else if (key == "depth_func:" &&
		         (!depth_func.has_value() || Priority > m_depth_func_priority))
		{
			std::string value = Input.getToken<std::string>(true, "\n\r\t;");
			const static std::unordered_map<std::string, uint32_t> values{
			...
			auto it = values.find(ToLower(value));
			if (it != values.end())
			{
				depth_func = it->second;
				m_depth_func_priority = Priority;
			}
			else
			{
				log_error("illegal depth func value: " + value);
			}
		}
		else if (key == "shader:" && (!shader || Priority > m_shader_priority))
		{
			try
			{
		...
		else
		{
			...
			if (value == "{")
			{
				// unrecognized or ignored token, but comes with attribute block and potential
				// further nesting go through it and discard the content
	...
	return true; // return value marks a key: value pair was extracted, nothing about whether it's
	             // recognized
}
...
bool opengl_material::is_translucent() const
{
	...
	return (textures[0] != null_handle ? GfxRenderer->Texture(textures[0]).has_alpha : false);
}

// create material object from data stored in specified file.
// NOTE: the deferred load parameter is passed to textures defined by material, the material itself
// is always loaded immediately
material_handle material_manager::create(std::string const &Filename, bool const Loadnow)
{
	...
	auto const locator{isgenerated ? std::make_pair(filename, "make:") : find_on_disk(filename)};
	...
	else
	{
		...
		if (texturehandle != null_handle)
		{
			...
			/*
			            // material would attach default shader anyway, but it would spit to error
			   log try
			            {
			...
			*/
			...
			cParser materialparser("texture1: \"" + Filename + "\"", cParser::buffer_TEXT);
