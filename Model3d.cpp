...
void TSubModel::SetMatrix(const glm::mat4 &matrix)
{
	b_Anim = TAnimType::at_External;
	b_aAnim = TAnimType::at_External;
	transform = matrix;
				...
				case 0:
				{
					...
					for (auto &vertex : submodel.Vertices)
					{
						...
						if (submodel.eType < TP_ROTATOR)
						{
							...
							if ((false == submodel.m_normalizenormals) && (std::abs(glm::length2(vertex.normal) - 1.0f) > 0.01f))
							{
